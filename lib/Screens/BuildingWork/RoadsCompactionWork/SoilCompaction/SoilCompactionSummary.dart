...
class _SoilCompactionSummaryState extends State<SoilCompactionSummary> {
  // Initialize using lazyPut
  final SoilCompactionViewModel soilCompactionViewModel = Get.find<SoilCompactionViewModel>();

  List filteredSoilList = [];

  @override
  void initState() {
    super.initState();
    // Initially show all data
    filteredSoilList = soilCompactionViewModel.allSoil;
  }

  // Function to apply the filter based on fromDate, toDate, and block
  void _applyFilter(DateTime? fromDate, DateTime? toDate, String? block) {
    setState(() {
      filteredSoilList = soilCompactionViewModel.allSoil.where((entry) {
        bool matchesBlock = block == null || block.isEmpty || (entry.block_no != null && entry.block_no!.contains(block));
        bool matchesFromDate = fromDate == null || (entry.start_date != null && entry.start_date!.isAfter(fromDate.subtract(Duration(days: 1))));
        bool matchesToDate = toDate == null || (entry.expected_comp_date != null && entry.expected_comp_date!.isBefore(toDate.add(Duration(days: 1))));
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      ...
                      DataColumn(label: Text('total_length'.tr(),
                          style: const TextStyle(fontWeight: FontWeight.bold))),
                      DataColumn(label: Text('Working Hours'.tr(),
                          style: const TextStyle(fontWeight: FontWeight.bold))),
                    ...
                    rows: filteredSoilList.map((entry) {
