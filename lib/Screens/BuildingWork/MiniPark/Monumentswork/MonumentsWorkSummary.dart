...
class _MonumentsWorkSummaryState extends State<MonumentsWorkSummary> {
  final MonumentsWorkViewModel monumentsWorkViewModel = Get.put(MonumentsWorkViewModel());
  ...
  List<dynamic> getFilteredData() {
    if (fromDate == null && toDate == null) {
      return monumentsWorkViewModel.allMonument;
    }

    return monumentsWorkViewModel.allMonument.where((entry) {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      rows: filteredData.map((entry) {
                        ...
                        return DataRow(
                          cells: [
                            DataCell(Text(startDate, style: const TextStyle(fontSize: 12))),
                            DataCell(Text(expectedCompDate, style: const TextStyle(fontSize: 12))),
                            DataCell(Text(entry.monumentsWorkCompStatus ?? '', style: const TextStyle(fontSize: 12))),
                            DataCell(Text(entry.date ?? '', style: const TextStyle(fontSize: 12))),
                            DataCell(Text(entry.time ?? '', style: const TextStyle(fontSize: 12))),
                          ],
                        );
                      }).toList(),
                    ),
